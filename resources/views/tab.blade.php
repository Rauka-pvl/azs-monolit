...
@section('content')
    ...
    <button id="hiddenButton" class="d-none"></button>
    <!-- Модальное окно -->
    <div id="myModal" class="modal">
        <div class="modal-content">
            ...
            <div class="justify-content-center">
                <div>
                    <label id='error_exit' class="center" style="color: red; display: none;"></label>
                </div>
                <div class="row mb-3">
                    <label class="col-md-4 col-form-label text-md-end"
                        style="font-weight: bold; font-size: 1.25em;">{{ __('PIN - код') }}</label>
                    <div class="col-md-6">
                        <input disabled id="input_pin_exit" type="text" class="form-control" required maxlength="6"
                            minlength="6" style="font-size: 2em; max-width: 250px;">
                    </div>
                    <div class="center pin-cal" id="pin-cal-exit" style="text-align: center;">
                        <div>
                            <button class="btn pin_num_exit">1</button>
                            <button class="btn pin_num_exit">2</button>
                            <button class="btn pin_num_exit">3</button>
                        </div>
                        <div>
                            <button class="btn pin_num_exit">4</button>
                            <button class="btn pin_num_exit">5</button>
                            <button class="btn pin_num_exit">6</button>
                        </div>
                        <div>
                            <button class="btn pin_num_exit">7</button>
                            <button class="btn pin_num_exit">8</button>
                            <button class="btn pin_num_exit">9</button>
                        </div>
                        <div>
                            <button class="btn" style="color: white;">0</button>
                            <button class="btn pin_num_exit">0</button>
                            <button id="deleteButton_exit" style="font-size: 1em;" class="btn">
                ...
                <div class="row mb-0">
                    <div class="center">
                        <button onclick="exit()" class="btn btn-primary">
                            {{ __('Выход') }}
                        </button>
                    </div>
                </div>
            ...
            <div class="card" style="margin: 2em auto; width: 45%; border: 0; ">
                <div style="text-align: center;">
                    ...
                    <div class="center" style="margin-top: 0.5em;">
                        <h4 id="address" style="margin-bottom: 0;"></h4>
                    </div>
                    <div class="center" style="margin-top: 0.5em;">
                ...
                <div class="grades">
                    ...
                    <button id="gradeB1" onclick="gradeKab(this)" name="grade" value="1" class="btn b_star">
                        <img src="{{ Storage::url('icons/star.png') }}" height="50">
                    </button>
    ...
    <div class="justify-content-center" id="auth">
        <div class="card" style="margin: 1em auto; width: 45%;">
            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <label class="col-md-4 col-form-label text-md-end"
                        style="font-weight: bold; font-size: 1.40em;">{{ __('PIN - код') }}</label>
                    <div class="col-md-6">
    ...
    <script>
        ...
        function updateUI(response) {
            ...
            if (response['photo'] != null)
                ...
                $("#photo").attr("src", '{{ Storage::url('photo/pupil.png') }}');
        ...
        function qr(user_id) {
            ...
            mergeImages([{
                ...
                {
                    src: '{{ Storage::url('icons/water-bg.png') }}',
                    x: 125,
                    y: 125,
                    width: 300,
                    height: 300,
        ...
        // Выход
        $('.pin_num_exit').on('click', function() {
            let value = $(this).text();
            let currentValue = $('#input_pin_exit').val();

            if (currentValue.length < 6) {
                $('#input_pin_exit').val(currentValue + value);
            }
        });
        $('#deleteButton_exit').on('click', function() {
            var currentValue = $('#input_pin_exit').val();
            var newValue = currentValue.slice(0, -1);
            $('#input_pin_exit').val(newValue);
        });
        ...
        function saveDataToLocalStorage(data) {
            ...
            expirationDate.setHours(time[0]);
            expirationDate.setMinutes(time[1]);
        ...
        function exit() {
            ...
            if (data) {
                let codeExit = $("#input_pin_exit").val();
                if (data['code'] == codeExit) {
                    localStorage.removeItem('savedData');
                    location.reload();
                } else {
                    console.log('eeeee');
                    $("#error_exit").text("Не правильный пин код!");
                    $("#error_exit").css('display', 'block');
                }
                setTimeout(() => {
                    $("#error_exit").css('display', 'none');
                }, 3000)
            }
        }
    </script>
